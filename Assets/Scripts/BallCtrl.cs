...
public class BallCtrl : MonoBehaviour
{
    ...
    [HideInInspector] public Teleport lastPortal = null;
    [HideInInspector] public int teleportCount = 1;
    ...
    void TrySwipe(Vector2 delta)
    {
        ...
        {
            if(GameManager.instance != null) GameManager.instance.BallMoveCountUpdate(1);
    ...
    IEnumerator MoveTo(Vector2 target)
    {
        ...
        while ((rb.position - target).sqrMagnitude > 0.0001f && !isTeleporting)
        {
            Vector2 next = Vector2.MoveTowards(rb.position, target, moveSpeed * Time.deltaTime);
            rb.MovePosition(next);
            yield return null;
        }
        if(!isTeleporting) rb.MovePosition(target);
        isMoving = false;
    }
    public void StopAllCouroutinesBall()
    {
        isMoving = false;
        StopAllCoroutines();
    }
    ...
    public void UndoMove()
    {
        ...
        GameManager.instance.BallMoveCountUpdate(-1);
    }
